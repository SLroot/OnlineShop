...
public record OrderResponse(
    ...
    public record CustomerInfo(Long id, String fullName, String email) {}

    public record PaymentInfo(
        Long id, String status, BigDecimal amount,
        String transactionRef, String failureReason,
        Instant paidAt, Instant refundedAt
    ) {}

    /** برای کاربر — بدون اطلاعات مشتری چون خودش است */
    public static OrderResponse from(Order order) {
        return build(order, false);
    }

    /** برای مدیر — با اطلاعات مشتری */
    public static OrderResponse withCustomer(Order order) {
    ...
    private static OrderResponse build(Order order, boolean includeCustomer) {
        ...
        return new OrderResponse(
                ...
                order.getShippingAddress(), items, paymentInfo,
                order.getPaymentDeadline(), order.getCancellationReason(),
                customer, order.getCreatedAt());
    }
    ...
    private static OrderItemResponse toItem(OrderItem item) {
        ...
        return new OrderItemResponse(
                ...
                item.getStatus().name(), item.getCancellationReason(),
